...
import trimesh  # noqa: F401  (kept to preserve upstream import surface)
from omegaconf import DictConfig, OmegaConf

# ``imageio`` and ``rembg`` are only used by ``save_video`` / ``remove_background``
# respectively. They drag in heavy native dependencies, so we import lazily.


def parse_structured(fields: Any, cfg: dict | DictConfig | None = None) -> Any:
    scfg = OmegaConf.merge(OmegaConf.structured(fields), cfg)
    return scfg


def find_class(cls_string):
    # The upstream config.yaml references classes via ``tsr.<...>`` paths. We
    # vendored the package under ``meshmind._tsr``, so transparently rewrite the
    # prefix at lookup time.
    if cls_string.startswith("tsr."):
        cls_string = "meshmind._tsr." + cls_string[len("tsr."):]
...
class BaseModule(nn.Module):
    ...
    def __init__(
        self, cfg: dict | DictConfig | None = None, *args, **kwargs
    ) -> None:
        super().__init__()
...
class ImagePreprocessor:
    ...
    def __call__(
        self,
        image: PIL.Image.Image | np.ndarray | torch.FloatTensor | list[PIL.Image.Image] | list[np.ndarray] | list[torch.FloatTensor],
        size: int,
    ) -> Any:
...
def chunk_batch(func: Callable, chunk_size: int, *args, **kwargs) -> Any:
    ...
    out_merged: dict[Any, torch.Tensor | None] = {}
...
def remove_background(
    ...
    **rembg_kwargs,
    ...
    if do_remove:
        import rembg  # local import: heavy dep, optional at runtime
...
def save_video(
    ...
    import imageio  # local import: optional dep, only needed for video export
